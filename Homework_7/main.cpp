...
node *two34_tree::find_1(int k)
{
	...
	if (ptr->is_leaf != 1)
	{
		...
		if (p->num_values == 3)
		{
			int temp = ptr->value[v4];
			ptr->value[v4] = p->value[3];
			p->value[3] = temp;
			return p;
		}
	...
	node *prt = ptr->parent;
	int state = ptr->child_state;
	int state_1 = state - 1;
	int r = 1;
	if (state > -1)
	{
		if (prt->child[state_1]->num_values > 1)
		{
			int k1 = prt->child[state_1]->num_values;
			prt->child[state_1]->value[k1] = -999;
			prt->child[state_1]->num_values = prt->child[state_1]->num_values - 1;
			ptr->num_values = ptr->num_values + 1;
			ptr->value[2] = ptr->value[1];
			ptr->value[1] = prt->value[state];
			prt->value[state] = v2;
			r = 0;
		}
	}
	...
	{
		state_1 = state_1 + 2;
		if (state_1 <= prt->num_values)
		{
			if (prt->child[state_1]->num_values > 1)
			{
				int k2 = prt->child[state_1]->value[1];
				prt->child[state_1]->value[1] = prt->child[state_1]->value[2];
				prt->child[state_1]->value[2] = prt->child[state_1]->value[3];
				prt->child[state_1]->value[3] = -999;
				prt->child[state_1]->num_values = prt->child[state_1]->num_values - 1;
				ptr->num_values = ptr->num_values + 1;
				ptr->value[2] = prt->value[state + 1];
				prt->value[state + 1] = k2;
				r = 0;
			}
		}
	}
