...
void heap_decrease_key(vector<rt_node> &rt, vector<h_node> & heap, int id,
		int n);
h_node heap_extrat_min(vector<rt_node> & rt, vector<h_node> & heap);
void prim(vector<vector<e_node> > & graph, vector<rt_node> & rt,
		vector<h_node> &heap);
...
void min_heapify(vector<rt_node> &rt, vector<h_node> & heap, int index) {
	int size = heap.size();
	int l = 0, r = 0, smallest = 0;
	l = index * 2 + 1;
	r = index * 2 + 2;
	int tmp;
	if (l < size && heap[l].cost < heap[index].cost)
		smallest = l;
	else
		smallest = index;
	if (r < size && heap[r].cost < heap[smallest].cost)
		smallest = r;
	if (smallest != index) {
		rt[heap[index].id].h_pos = smallest;
		rt[heap[smallest].id].h_pos = index;
		tmp = heap[index].cost;
		heap[index].cost = heap[smallest].cost;
		heap[smallest].cost = tmp;
		tmp = heap[index].id;
		heap[index].id = heap[smallest].id;
