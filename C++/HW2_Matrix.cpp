...
void my_matrix::add_node(int v, int r, int c)
{
    ...
    if (r_p != nullptr) {
        while (r_p->r_next != nullptr
            && r_p->r_next->c_position < c) {
            r_p = r_p->r_next;
        }
    }

    if (c_p != nullptr) {
        while (c_p->c_next != nullptr
            && c_p->c_next->r_position < r) {
            c_p = c_p->c_next;
        }
    }
...
void my_matrix::better_print() const
{
    cout << "M=" << endl;
cout << "[" << endl;
