...
void heap_adjust_down(vector<rt_node> & rt, vector<h_node> & heap)
{
	...
	if( heap.size() == 2 )//there are only two node in the heap
		{
	...
	while ( currentChild < heap.size() ) {
		if (heap[currentNode].cost > heap[currentChild].cost) { //exchange nodes
...
void heap_op(vector<vector<e_node> > & graph, vector<rt_node> & rt, vector<h_node> &heap) {
	...
	for (size_t i = 0; i < rt.size(); i++ )
	{
		...
		for (auto currentNeighbor : graph[checkedNode])
		{
			...
			{
					rt[neighborId].cost = rt[checkedNode].cost + currentNeighbor.weight;//update cost
